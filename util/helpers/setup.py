...
def checkpoint(path, f, is_best=False):
    """
    When passing is_best as True, stores a separate copy of model. Used for pretraining, where looking to save model that shows best performance.
    """
    torch.save(f.state_dict(), os.path.join(path, "f.pth"))
    if is_best:
        shutil.copyfile(os.path.join(path, "f.pth"), os.path.join(path, "best.pth"))
